...
rule calc_EAWMI_zonal_u300hPa:
    ...
    wildcard_constraints:
        ...
        # data_folder=config['download_path'] + '|' + config['intermediate_files']
    threads: 1
    run:
...
rule calc_MO_mslp_index:
    input:
        mslp_data = config['intermediate_files']+"/era5.single_level.mean_sea_level_pressure.DJF.{sdate}-{edate}.nc"
    output:
        outpath = config['EAWMI_path']+"/era5.single_level.EAWM_MO.DJF.{sdate}-{edate}.nc"
    run:
